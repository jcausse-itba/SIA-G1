...
def hcl_to_lab(h: float, c: float, l: float) -> np.ndarray:
    """
    Convierte las componentes HCL directamente a un vector CIELAB [L*, a*, b*].
    H: [0, 360], C: [0, 130], L: [0, 100]
    """
    h_rad = np.radians(h)
    a = c * np.cos(h_rad)
    b = c * np.sin(h_rad)
...
def hcl_to_rgba_int(h: float, c: float, l: float, alpha: float) -> tuple[int, int, int, int]:
    """
    Convierte HCL -> Lab -> XYZ -> sRGB lineal -> sRGB [0, 255]
    Solo se utiliza al exportar o renderizar la imagen final a RGB.
    """
    # 1. HCL a Lab
    h_rad = np.radians(h)
    a = c * np.cos(h_rad)
    ...
    # 4. sRGB Lineal a sRGB Comprimido
...
def rgb_to_lab_vectorized(rgb_img: np.ndarray) -> np.ndarray:
    """
    Convierte una imagen NumPy RGB (H, W, 3) en rango [0, 255] a CIELAB.
    Se ejecuta una sola vez al instanciar el evaluator para precalcular el target.
    """
